...
class Camera : public OpticalDevice{
   ...
   char* shootingMode; // режими зйомки
public:
   Camera(const char* model, double focalLength, double aperture, double wt,
          double megapixels, const char* mode);
   Camera(const Camera& other, const char* name);
   ~Camera();
   void capturePhoto() const; // метод для зйомки
   void printInfo() const; // перевизначив
};
